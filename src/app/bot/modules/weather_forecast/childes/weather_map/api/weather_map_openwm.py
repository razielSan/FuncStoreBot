...
from app.bot.modules.weather_forecast.childes.weather_map.extensions import (
    folium_weather_map,
)
...
class WeatherMapOpenWMApi:
    async def get_weather_map(
        ...
        )  # Делаем проверку на доступность сайта

        if weather_map.error:
            return weather_map

        path: Path = await folium_weather_map(
            ...
            path=path_to_weathermap,
            api_openweathermap=api_openweathermap,
        )  # делаем запрос на формирование карты через библиотеку follium
